...
class DocstringAdder:
    ...
    def process_file(self, file_path: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            existing_count = (1 if module_docstring else 0) + len(class_docstrings) + len(function_docstrings)
            ...
            if existing_count >= total_count * 0.75:  # 75% coverage threshold
                return True, f"File already has good docstring coverage ({existing_count}/{total_count})"
            ...
            for match in re.finditer(r'class\s+(\w+)(?:\(.*?\))?:', content):
                ...
                if not any(doc['name'] == class_name for doc in class_docstrings):
                    ...
                    class_pos = match.end()
                    docstring = f'    """{self._generate_class_docstring(class_name)}"""'
                    
                    new_content = new_content[:class_pos] + '\n' + docstring + '\n    ' + new_content[class_pos:]
            ...
            for match in re.finditer(r'def\s+(\w+)\s*\((.*?)\)(?:\s*->.*?)?:', content):
                ...
                if (any(doc['name'] == func_name for doc in function_docstrings) or 
                    (func_name.startswith('__') and func_name.endswith('__') and func_name != '__init__')):
                    continue
                ...
                new_content = new_content[:func_pos] + '\n' + docstring + '\n' + indent + '    ' + new_content[func_pos:]
    ...
    def _extract_function_docstrings(self, content: str) -> List[Dict[str, str]]:
        ...
        """
        function_pattern = r'def\s+(\w+)\s*\((.*?)\)(?:\s*->.*?)?:\s*(?:"""([\s\S]*?)""")?'
        functions = []
