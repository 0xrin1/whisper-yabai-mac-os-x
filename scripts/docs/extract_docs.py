...
class DocExtractor:
    ...
    def extract_function_docstrings(self, content: str) -> List[Dict[str, str]]:
        ...
        for match in re.finditer(function_pattern, content):
            ...
            if func_name.startswith('__') and (not docstring or func_name != '__init__'):
                continue
    ...
    def docstring_to_markdown(self, docstring: str) -> str:
        ...
        while i < len(lines):
            ...
            if re.match(r'^(Args|Arguments|Parameters|Returns|Raises|Yields|Examples|Notes):', line):
    ...
    def generate_markdown_for_module(self, module_data: Dict[str, Any], module_path: str) -> str:
        """
        ...
        if module_data.get('module_docstring'):
            markdown += f"{self.docstring_to_markdown(module_data['module_docstring'])}\n\n"
        ...
        for func_data in module_data.get('functions', []):
            markdown += f"## Function: `{func_data['name']}({func_data['parameters']})`\n\n"
        ...
        def process_directory(directory, path_prefix="", indent=""):
            ...
            for item_name, item_data in sorted(directory.items()):
                if isinstance(item_data, dict) and 'path' not in item_data:
                    ...
                    process_directory(item_data, f"{path_prefix}{item_name}/", f"{indent}  ")
                else:
                    # This is a file
                    ...
                    output_rel_path = f"{rel_path}/{module_name}.md" if rel_path else f"{module_name}.md"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Extract documentation from Python files and generate markdown for mdBook")
    parser.add_argument('--base-dir', default='src', help="Base directory containing the code")
    parser.add_argument('--output-dir', default='docs-src/src', help="Output directory for the markdown files")
