...
class AudioRecorder:
    ...
    def start_recording(
        ...
        try:
            ...
            if duration is None:
                if trigger_mode:
                    ...
                    duration = (
                        2.0  # 2.0 seconds for more reliable trigger word detection
                    )
                elif dictation_mode:
            ...
            try:
                ...
                handle_error(
                    file_error,
                    logger,
                    context="Temporary file creation",
                    notification_text="Failed to create temporary file",
                    should_raise=True,
                )
            ...
            try:
                ...
                for i in range(num_devices):
                    if (
                        ...
                        device_name = self.p.get_device_info_by_host_api_device_index(
                            0, i
                        ).get("name")
                ...
                try:
                    ...
                    logger.debug(
                        f"Default input device: {device_name} (index: {default_input_device_index})"
                    )
                except IOError as device_error:
                    error_msg = (
                        f"No default input device found: {device_error}. "
                ...
                try:
                    ...
                    if len(test_data) == 0:
                        logger.warning(
                            "Audio stream test got empty data - microphone may not be working properly"
                        )
                    else:
                        logger.debug(
                            f"Audio stream test successful, read {len(test_data)} bytes"
                        )
                except Exception as read_error:
                    handle_error(
                        read_error,
                        logger,
                        context="Audio stream test",
                        notification_text="Failed to read from microphone",
                        should_raise=True,
                    )
                    stream.close()
                    state.stop_recording()
            ...
            handle_error(
                ...
                notification_text="Failed to start recording",
                should_raise=False,
            )
            state.stop_recording()
            return None
        ...
        max_duration = (
            60 if dictation_mode else 15
        )  # 60 seconds for dictation, 15 for commands
        total_frames = int(frames_per_second * max_duration)

        # Minimum recording duration - increased to ensure we get enough audio
        if trigger_mode:
            min_duration = (
                1.5  # Trigger detection at least 1.5 seconds for better detection
            )
        elif dictation_mode:
            min_duration = (
            ...
            min_duration = (
                4  # At least 4 seconds for commands to avoid premature cutoff
            )

        min_frames = int(frames_per_second * min_duration)

        logger.debug(
            f"Beginning smart recording (max: {max_duration}s, min: {min_duration}s, silence timeout: {max_silence_seconds}s)"
        )
        ...
        try:
            ...
            while frames_recorded < total_frames and state.is_recording():
                try:
                    ...
                    if frames_recorded % int(frames_per_second) == 0:
                        ...
                        logger.debug(
                            f"Recorded {seconds_recorded:.1f} sec (real: {elapsed_real:.1f}s), "
                            f"Energy: {energy:.0f}"
                        )
                        ...
                        if (
                            ...
                            if not trigger_mode and not dictation_mode:
                                ...
                                if seconds_recorded < min_seconds_required:
                                    ...
                                    logger.debug(
                                        f"Ignoring silence detection as we need at least {min_seconds_required}s "
                            ...
                            logger.debug(
                                f"Stopping recording after detecting {silence_frames/frames_per_second:.1f}s of silence"
                            )
                            break
            ...
            handle_error(
                rec_loop_err,
                logger,
                context="Recording loop",
                notification_text="Recording interrupted",
                should_raise=False,
            )
        ...
        logger.debug(
            f"Recording complete - {frames_recorded} frames in {elapsed:.2f} seconds"
        )

        # If we recorded for less than 80% of the intended time, log a warning
        if elapsed < (duration * 0.8) and state.is_recording():
            logger.warning(
                f"Recording completed prematurely: {elapsed:.2f}s instead of {duration}s"
            )
        ...
        if frames_recorded == 0:
            ...
            try:
                ...
                handle_error(
                    cleanup_err,
                    logger,
                    context="Stream cleanup after empty recording",
                    notification_text=None,
                    should_raise=False,
                )
        ...
        try:
            ...
            if os.path.exists(temp_filename):
                ...
                if file_size < 1000:
                    logger.warning(
                        "WAV file seems very small, may not contain enough audio"
                    )
            else:
            ...
            handle_error(
                wav_err,
                logger,
                context="Saving WAV file",
                notification_text="Failed to save recording",
                should_raise=True,
            )
            state.stop_recording()
            return None
        ...
        try:
            ...
            if not trigger_mode:  # Don't show for trigger recordings
                try:
                    ...
                    handle_error(
                        notify_err,
                        logger,
                        context="Recording completion notification",
                        notification_text=None,
                        should_raise=False,
                    )
            ...
            handle_error(
                queue_err,
                logger,
                context="Enqueueing audio",
                notification_text="Failed to process recording",
                should_raise=False,
            )
