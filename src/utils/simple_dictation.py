...
from src.audio.resource_manager import audio_device, audio_stream, TempAudioFile, play_system_sound
        ...
        try:
            with audio_device() as p:
                ...
                with wave.open(temp_file.filename, 'wb') as wf:
                    ...
                    wf.setsampwidth(p.get_sample_size(getattr(p, config.get('FORMAT', 'paInt16'))))
    ...
    try:
        ...
        if KEY_STATES['ctrl'] and KEY_STATES['shift'] and KEY_STATES['d']:
            ...
            def record_and_transcribe():
                ...
                audio_file = recorder.start_recording(duration=config.get('DICTATION_TIMEOUT', 5))
                if audio_file:
