...
class CommandProcessor:
            ...
            if "pyautogui." in cmd_str:
                try:
                    ...
                    if "pyautogui.hotkey" in cmd_str:
                        ...
                        text = (
                            cmd_str.split("pyautogui.write(")[1]
                            .split(")")[0]
                            .strip("'\" ")
                        )
    ...
    def parse_and_execute(self, text):
        ...
        for fragment in dictation_fragments:
            if fragment in clean_text:
                logger.info(
                    f"Detected dictation command: '{fragment}' in '{clean_text}'"
                )
        ...
        if command in self.commands:
            ...
            logger.warning(
                f"Unknown command: {command}, looking in full text for commands"
            )
    ...
    def focus_application(self, args):
        ...
        return self.execute_shell_command(
            f"yabai -m window --focus \"$(/usr/bin/grep -i '{app_name}' <(/usr/bin/yabai -m query --windows | /usr/bin/jq -r '.[].app') | head -n1)\""
        )
    ...
    def resize_window(self, args):
        ...
        if direction in ["left", "right", "top", "bottom"]:
            return self.execute_shell_command(
                f"yabai -m window --resize {direction}:20:20"
            )
        else:
            logger.warning(f"Unknown direction: {direction}")
            return False
