...
class ContinuousRecorder:
    ...
    def _recording_thread(self):
        ...
        try:
            ...
            logger.debug(f"Continuous recording using input device index: {default_input_device_index}")
            ...
            while self.running:
                ...
                if state.is_recording():
                    logger.debug("Pausing continuous recording while active recording is in progress")
                    time.sleep(0.1)
                    continue
                ...
                try:
                    ...
                    if len(state.audio_buffer) % int(self.frames_per_second * 2) == 0:
                        logger.debug(f"Audio energy level: {energy:.0f}, buffer size: {len(state.audio_buffer)}")
                    
                    # Detect speech activity
                    if energy > self.energy_threshold:
                        # High energy detected - could be speech
                        if not has_speech:
                            logger.debug(f"Voice activity detected, energy: {energy:.0f}")
                            has_speech = True
                        silence_frames = 0
                    else:
                        ...
                        if has_speech and silence_frames >= max_silence_frames:
                            logger.debug(f"Potential trigger word - processing buffer after {silence_frames/self.frames_per_second:.1f}s silence")
                            ...
                            if not state.is_recording():
                                ...
                                logger.debug("Skipping buffer processing - already recording")
    ...
    def _process_buffer(self):
        ...
        try:
            ...
            with state.audio_buffer_lock:
                if len(state.audio_buffer) < 10:  # At least some minimum amount of frames
