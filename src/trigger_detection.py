...
class TriggerDetector:
    ...
    def __init__(self, whisper_model=None):
        ...
        self.command_variations = [
            state.command_trigger.lower(), "hay", "he", "hey", "hi", "okay", "ok"
        ]
        
        self.dictation_variations = [
            state.dictation_trigger.lower(), "typing", "write", "note", "text", "speech to text",
    ...
    def detect_triggers(self, transcription):
        ...
        for variation in self.dictation_variations:
            ...
            if (f" {variation} " in f" {transcription.lower()} " or
                ...
                logger.debug(f"WORD BOUNDARY MATCH found for '{variation}' in '{transcription}'")
                contains_dictation_trigger = True
                break
                
            # Check for substring match
            if variation in transcription.lower():
                logger.debug(f"SUBSTRING MATCH found for '{variation}' in '{transcription}'")
                contains_dictation_trigger = True
                break
