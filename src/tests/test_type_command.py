...
class TypeTriggerTest(unittest.TestCase):
        ...
        try:
            ...
            while time.time() - start_time < timeout:
                ...
                if len(current_lines) > initial_line_count:
                    ...
                    logger.info(f"Found {len(new_entries)} new entries in dictation log")
                    return new_entries
                    
                time.sleep(1)
                
            logger.warning(f"No new entries found in dictation log after {timeout} seconds")
            return []
    ...
    def test_type_trigger_complete_flow(self):
        ...
        try:
            ...
            for trigger_phrase in test_variations:
                ...
                dictation_activated = self.check_daemon_output("DICTATION TRIGGER DETECTED")
                
                if dictation_activated:
                    logger.info(f"SUCCESS: Dictation mode activated with trigger phrase: '{trigger_phrase}'")
                    triggered = True
                    ...
                    applescript_detected = self.check_daemon_output("Using AppleScript keystroke method")
                        ...
                        if os.path.exists("dictation_log.txt"):
                            with open("dictation_log.txt", "r") as f:
                                ...
                                if test_phrase.lower() in log_content.lower():
                                    logger.info(f"Found test phrase in dictation log: '{test_phrase}'")
                    ...
                    if not applescript_detected:
                        logger.warning("AppleScript execution was not detected after dictation")
                ...
                transcription_match = re.search(r"Buffer transcription: '([^']+)'", content)
                if transcription_match:
                    transcription = transcription_match.group(1)
                    logger.info(f"Daemon transcribed: '{transcription}'")
                    self.test_results["transcriptions"] = self.test_results.get("transcriptions", [])
                ...
                for transcription in transcriptions:
                    ...
                    for fragment in trigger_fragments:
                        if fragment in transcription.lower():
                            logger.info(f"Found trigger fragment '{fragment}' in transcription: '{transcription}'")
                            triggered = True
                            break
            ...
            if not triggered:
                logger.warning(f"No trigger phrases worked for dictation mode: {test_variations}")
        ...
        try:
            ...
            for i in range(sequences):
                ...
                for phrase in trigger_phrases:
                    ...
                    # Check if dictation mode was activated
                    if self.check_daemon_output("DICTATION TRIGGER DETECTED", timeout=10):
                        logger.info(f"Dictation mode activated with phrase '{phrase}' in sequence {i+1}")
                        trigger_detected = True
                        break
                    else:
                        logger.warning(f"Dictation not triggered with '{phrase}' in sequence {i+1}")
                        # Add a pause between attempts
                        time.sleep(3)
                
                if not trigger_detected:
                    logger.warning(f"Failed to activate dictation mode in sequence {i+1} with any phrase")
                    continue
                    
                # Now send a test phrase
                test_phrase = f"Test phrase for sequence {i+1} testing Apple Script execution"
                logger.info(f"Sending test phrase: '{test_phrase}'")
                ...
                if self.check_daemon_output("Running AppleScript", timeout=10):
                    ...
                    logger.warning(f"AppleScript execution not detected in sequence {i+1}")
            ...
            if successful_sequences == 0:
                ...
                logger.info(f"Successfully completed {successful_sequences} out of {sequences} sequences")
    ...
    def test_rapid_mode_switching(self):
        ...
        try:
            ...
            cmd_detected = self.check_daemon_output("COMMAND TRIGGER DETECTED", timeout=15)
            if cmd_detected:
                logger.info("Command trigger detected")
            else:
                logger.warning("Command trigger was not detected - continuing test anyway")
            ...
            dict_detected = self.check_daemon_output("DICTATION TRIGGER DETECTED", timeout=15)
            if dict_detected:
                logger.info("Dictation trigger successfully detected after command")
            else:
                logger.warning("Dictation trigger was not detected - continuing test anyway")
            ...
            if dict_detected:
                ...
                if script_executed:
                    ...
                    logger.warning("AppleScript execution not detected - continuing test anyway")
            ...
            if cmd2_detected:
                ...
                logger.warning("Second command not detected - test may not be conclusive")
    ...
    def test_applescript_execution_verification(self):
        ...
        try:
            ...
            temp_file_created = self.check_daemon_output("Saved text to /tmp/dictation_text.txt")
            ...
            success = self.check_daemon_output("AppleScript succeeded") or self.check_daemon_output("AppleScript") 
