...
class DaemonProcess:
    ...
    def contains_output(self, text, last_n_lines=None):
        ...
        if last_n_lines is not None:
            lines_to_check = self.output_lines[-last_n_lines:] if self.output_lines else []
    ...
    def direct_feed_to_buffer(self, audio_file):
        ...
        try:
            ...
            if wf.getnchannels() != 1 or wf.getframerate() != 16000:
                logger.warning(f"Audio format incompatible: channels={wf.getnchannels()}, rate={wf.getframerate()}")
                logger.warning("Audio should be 16kHz mono for best results with Whisper")
...
class VoiceControlTests(unittest.TestCase):
        ...
        for phrase in variations:
            ...
            for voice in voices:
                try:
                    ...
                    if self.daemon.contains_output("DICTATION TRIGGER DETECTED", last_n_lines=100):
                        logger.info(f"SUCCESS: Detected dictation trigger with '{phrase}' using voice {voice}")
                        detected = True
                        os.remove(audio_file)
    ...
    def test_3_command_mode_functionality(self):
        ...
        self.assertTrue(
            self.daemon.contains_output("Detected open command for app: safari", last_n_lines=100) or
            self.daemon.contains_output("Opening application: safari", last_n_lines=100),
    ...
    def test_4_dictation_mode_functionality(self):
        ...
        dictation_file = self.synth.synthesize_speech("This is a test of dictation mode")
    ...
    def test_5_bug_recording_flag_reset(self):
        ...
        self.assertTrue(
            self.daemon.contains_output("Maximizing window", last_n_lines=200) and 
            (self.daemon.contains_output("Opening application: terminal", last_n_lines=100) or
             self.daemon.contains_output("Detected terminal command", last_n_lines=100)),
    ...
    def test_6_rapid_dictation_switching(self):
        ...
        self.assertTrue(
            self.daemon.contains_output("DICTATION TRIGGER DETECTED", last_n_lines=500) and
            self.daemon.contains_output("Maximizing window", last_n_lines=300) and
            self.daemon.contains_output("Dictation completed successfully", last_n_lines=100),
    ...
    def test_7_background_noise_handling(self):
        ...
        noise_file = self.synth.synthesize_speech("one two three four five six seven eight nine ten")
        ...
        self.assertFalse(
            self.daemon.contains_output("COMMAND TRIGGER DETECTED", last_n_lines=20) or
            self.daemon.contains_output("DICTATION TRIGGER DETECTED", last_n_lines=20),
    ...
    def test_9_dictation_recording_conflict(self):
        ...
        for i in range(3):
            ...
            has_dictation_trigger = self.daemon.contains_output("DICTATION TRIGGER DETECTED", last_n_lines=50)
            ...
            has_recording_started = self.daemon.contains_output("Setting RECORDING flag to True", last_n_lines=50) or \
                                   self.daemon.contains_output("recording mode", last_n_lines=50)
                                   
            self.assertTrue(has_recording_started, 
                           f"Recording didn't start after dictation trigger in iteration {i+1}")
                           
            # Now generate some speech to transcribe
            dictation_file = self.synth.synthesize_speech(f"This is test text for dictation iteration {i+1}")
            self.synth.play_audio_file(dictation_file)
            ...
            has_recording_completed = self.daemon.contains_output("recording completed", last_n_lines=60) or \
                                     self.daemon.contains_output("Dictation completed", last_n_lines=60)
                                     
            self.assertTrue(has_recording_completed, 
                           f"Recording didn't complete properly in iteration {i+1}")
                           
            # Verify that RECORDING flag was reset
            flag_reset = self.daemon.contains_output("RECORDING flag set to False", last_n_lines=60) or \
                        self.daemon.contains_output("Resetting RECORDING flag to False", last_n_lines=60)
                        
            self.assertTrue(flag_reset, 
                           f"RECORDING flag not reset in iteration {i+1}")
        ...
        self.assertTrue(
            self.daemon.contains_output("COMMAND TRIGGER DETECTED", last_n_lines=50) or
            self.daemon.contains_output("Maximizing window", last_n_lines=50),
