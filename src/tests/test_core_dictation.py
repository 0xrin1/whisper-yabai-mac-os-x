...
class TestCoreDictation(unittest.TestCase):
    ...
    def test_type_text_clipboard_failure(self):
        ...
        self.mock_subprocess.run.return_value = MagicMock(returncode=1)  # AppleScript fails
        self.mock_subprocess.Popen.side_effect = Exception("Failed to copy")  # pbcopy fails
    ...
    def test_all_methods_fail(self):
        ...
        self.mock_subprocess.run.return_value = MagicMock(returncode=1)  # AppleScript fails
        self.mock_subprocess.Popen.side_effect = Exception("Failed to copy")  # pbcopy fails
        self.mock_pyautogui.write.side_effect = Exception("PyAutoGUI failed")  # PyAutoGUI fails
    ...
    def test_completion_sound_disabled(self):
        ...
        self.mock_config.get.side_effect = lambda key, default=None: False if key == 'PLAY_COMPLETION_SOUND' else original_side_effect(key, default)
    ...
    def test_notification_disabled(self):
        ...
        original_side_effect = self.mock_config.get.side_effect
        self.mock_config.get.side_effect = lambda key, default=None: False if key == 'SHOW_DICTATION_NOTIFICATIONS' else original_side_effect(key, default)
